...
tree-sitter를 사용하여 소스 코드를 AST로 파싱하고, 언어별로 의미 있는 단위(함수, 클래스, 구조체 등)로 분할합니다.
...
4. **Import 필터링**: 각 청크에서 실제 사용하는 import만 선별하여 `imports` 필드에 포함
5. **커버리지 보완**: 추출된 청크가 커버하지 못하는 나머지 코드를 별도 청크로 수집 (200줄 초과 시 빈 줄 기준 분할)
