...
pub trait Language {
    ...
    /// import/use 문으로 취급할 노드 종류 (모든 청크의 imports에 포함됨)
...
fn lang_for_ext(ext: &str) -> anyhow::Result<Box<dyn Language>> {
    let lang: Box<dyn Language> = match ext {
        "rs" => Box::new(rust::Rust),
        "js" | "mjs" | "cjs" | "jsx" => Box::new(javascript::JavaScript),
        "ts" | "mts" | "cts" => Box::new(typescript::TypeScript),
        "tsx" => Box::new(typescript::Tsx),
        "py" | "pyi" => Box::new(python::Python),
        "go" => Box::new(go::Go),
