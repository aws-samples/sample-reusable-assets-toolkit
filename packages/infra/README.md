...
Run `pnpm nx build @idp-code/infra [--skip-nx-cache]` to build the application.

## Running unit tests

Run `pnpm nx test @idp-code/infra` to execute the unit tests via Vitest.

### Updating snapshots

To update snapshots, run the following command:

`pnpm nx test @idp-code/infra --configuration=update-snapshot`

## Run lint

Run `pnpm nx lint @idp-code/infra`

### Fixable issues

You can also automatically fix some lint errors by running the following command:

`pnpm nx lint @idp-code/infra --configuration=fix`

## Deploy to AWS

### Deploy all Stacks

Run `pnpm nx deploy @idp-code/infra --all`

### Deploy a single Stack

Run `pnpm nx deploy @idp-code/infra [stackName]`
...
Run `pnpm nx deploy @idp-code/infra --hotswap --all`
...
```typescript
import { suppressRules } from ':idp-code/common-constructs';
